...
describe('DarwinFinderBase', () => {
    ...
    let accessStub;
    beforeEach(() => {
        accessStub = (0, sinon_1.stub)(fs_1.promises, 'access').callsFake((path) => validPaths.has(path.toString()) ? Promise.resolve() : Promise.reject('no acccess'));
    });
    afterEach(() => {
        accessStub.restore();
    });
    class ReplacingRegisterFinder extends darwinEdge_1.DarwinEdgeBrowserFinder {
        constructor(file) {
            ...
            this.lsRegisterCommand = `cat "${(0, path_1.resolve)(__dirname, '..', 'fixtures', file)}"`;
        }
    }
    it('greps output on Mojave', async () => {
        const finder = new ReplacingRegisterFinder('edge-mojave-ungrepped.txt');
        (0, chai_1.expect)(await finder.findAll()).to.deep.equal([
            {
