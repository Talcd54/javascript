...
class DarwinFinderBase {
    constructor(env = process.env, fs = fs_1.promises, execa = execa) {
        this.env = env;
        this.fs = fs;
        this.execa = execa;
    ...
    async findWhere(predicate) {
        for (const test of this.wellKnownPaths) {
            if (predicate(test) && (await (0, util_1.canAccess)(this.fs, test.path))) {
                return test;
            }
        }
    ...
    findAll() {
        var _a;
        (_a = this.foundAll) !== null && _a !== void 0 ? _a : (this.foundAll = this.findAllInner());
    ...
    async findLaunchRegisteredApps(pattern, defaultPaths, suffixes) {
        const { stdout } = await this.execa.command(`${this.lsRegisterCommand} | awk 'tolower($0) ~ /${pattern.toLowerCase()}${pathSuffixRe.source}?$/ { $1=""; print $0 }'`, { shell: true, stdio: 'pipe' });
        const paths = [
            ...defaultPaths,
    ...
    createPriorities(priorities) {
        const home = this.env.HOME && (0, util_1.escapeRegexSpecialChars)(this.env.HOME);
        const preferred = this.getPreferredPath();
        ...
        if (preferred) {
            mapped.unshift({
                regex: new RegExp((0, util_1.escapeRegexSpecialChars)(preferred)),
                weight: 151,
