...
function checkDuplicate() {
    ...
    .trim()    //delete empty spaces front and back
    .replace(/\s{2,}/g, '')    //delete empty spaces in the word
    // Normalize the text for case-insensitive comparison
    let normalizedText = itemText.toLowerCase();    //put all entries to lower case
    // Add the cleaned item only if it's not a duplicate and not empty
    if (!listArr.some(item => item.toLowerCase() === normalizedText) && itemText !== "") {
...
//delete last entry
deleteItemButton.addEventListener('click', deleteLastEntry)
